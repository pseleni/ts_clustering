...
def run_set(dataset, ind_points, runs, metric = 'dtw'):
    ...
    print("Running time -SGPR: {:f} sec".format(run_time_base))
    ...
    for w in w_s:
        ...
        run_time = (end-start)/runs + run_time_base
    ...
    name = 'analytic/'+dataset+'_'+ metric+'_adj_rand_ind_preprocessed.csv'
    ...
    name = 'analytic/'+dataset+'_'+metric+'_adj_mutual_info_preprocessed.csv'
    with open(name, 'a', newline='') as f:
        writer = csv.writer(f)
...
def run_set_without(dataset, ind_points, runs, metric = 'dtw'):
    ...
    name = 'analytic/'+dataset+'_'+ metric+'_adj_rand_ind.csv'
    with open(name, 'a', newline='') as f:
        writer = csv.writer(f)
        wr = adj_rand_indices_random.copy()
        wr.insert(0, inducing_p)
    ...
    name = 'analytic/'+dataset+'_'+metric+'_adj_mutual_info.csv'
    with open(name, 'a', newline='') as f:
        writer = csv.writer(f)
        wr = adjusted_mutual_info_random.copy()
        wr.insert(0, inducing_p)
